...
<div class="min-h-screen bg-gray-900">
    <div class="max-w-2xl mx-auto px-4 py-12">
        ...
        <h1 class="text-2xl font-medium text-white mb-1">Schedule Match</h1>
        <p class="text-gray-400 text-sm mb-8">Add a new match for <strong>{{ $event->name }}</strong></p>
        ...
        <form method="POST" action="{{ route('user.events.matches.store', $event) }}"
            ...
            <div class="flex flex-col gap-1.5">
                ...
                @if($teams->isEmpty())
                <p class="text-orange-400 text-xs">⚠ No teams available for {{ $event->game->name }}. Create a team first.</p>
                @endif
                @error('team_a_id')
...
<script>
    ...
    function setMinDateTime() {
        ...
        document.getElementById('scheduledAt').value = oneHourLater.toISOString().slice(0, 16);
    }
